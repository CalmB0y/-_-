...
public class TrafficLightController : MonoBehaviour
{
    ...
    private void Start()
    {
        // Загрузка значений из PlayerPrefs или установка значений по умолчанию
        redDuration = PlayerPrefs.GetFloat("RedDuration", 5f); // Если ключа нет, будет 5 секунд
        yellowDuration = PlayerPrefs.GetFloat("YellowDuration", 2f);
        greenDuration = PlayerPrefs.GetFloat("GreenDuration", 5f);

        Debug.Log($"Red Duration: {redDuration}, Yellow Duration: {yellowDuration}, Green Duration: {greenDuration}");
